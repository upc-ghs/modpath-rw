...
contains
  ...
  subroutine ReadAndPrepareLocations(inUnit, outUnit, particleGroup, ibound,&
    ...
    integer :: inu, inputStyle, n, m
    integer :: offset, idmax
    ...
    ! Open starting locations file if inUnit is not positive.
    ! Otherwise, assume inUnit is open and read from it.
    if(inUnit .le. 0) then
      inu = 99
  ...
  subroutine pr_ReadLocations1(pGroup, inUnit, grid)
    ...
    integer :: totalParticleCount, layer, row, column, layerCount, rowCount, columnCount
  ...
  subroutine pr_ReadLocations2(pGroup, inUnit, grid, ibound, gridCellCount)
    ...
    integer :: totalParticleCount, templateCount, cellRegionCount
    integer :: np,n,m,i,layerCount,rowCount,columnCount,cell,npcell
  ...
  integer,intent(in) :: inUnit, gridCellCount
  integer,intent(in),dimension(gridCellCount) :: ibound
  integer :: totalParticleCount,templateCount,templateCellCount
  integer :: np,n,m,nn,i,layerCount,rowCount,columnCount,cell,offset,npcell
  integer,dimension(:),allocatable :: templateSubDivisionTypes,templateCellNumbers 
  integer,dimension(:),allocatable :: templateCellCounts, drape
  integer,dimension(:,:),allocatable :: subDiv
  integer,dimension(12) :: sdiv
  !-------------------------------------------------------------------------------------
  ...
  subroutine pr_ReadLocations4(pGroup, inUnit, outUnit, ibound, cellCount, grid)
    ...
    class(ModflowRectangularGridType),intent(in) :: grid
    ! local
    integer :: totalParticleCount, layerCount, maskValueCount, drape
    integer :: templateSubDivisionType, n, m, count, i, npcell
