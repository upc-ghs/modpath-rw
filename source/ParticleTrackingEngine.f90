...
!    doubleprecision :: StoppingTime = 0d0
    ...
    ! FlowModel
    type(FlowModelType), pointer :: flowModel
    ...
    type(BudgetListItemType),dimension(:), pointer :: ListItemBuffer
...
subroutine pr_Initialize(this,headReader, budgetReader, grid, hNoFlow, hDry, trackingOptions, flowModel)
!***************************************************************************************************************
!
!***************************************************************************************************************
!
...
!---------------------------------------------------------------------------------------------------------------
  ...
  type(FlowModelType), intent(in), target :: flowModel
...
!---------------------------------------------------------------------------------------------------------------
  ...
  ! Pointers to flow arrays
  this%FlowsRightFace => flowModel%FlowsRightFace
  this%FlowsFrontFace => flowModel%FlowsFrontFace
  this%FlowsLowerFace => flowModel%FlowsLowerFace
  this%FlowsJA        => flowModel%FlowsJA       
  ...
  ! FLOWMODEL
  this%Porosity             => flowModel%Porosity
  this%Retardation          => flowModel%Retardation
  this%Zones                => flowModel%Zones
  this%IBound               => flowModel%IBound 
  this%IBoundTS             => flowModel%IBoundTS             
  this%Heads                => flowModel%Heads                
  this%SourceFlows          => flowModel%SourceFlows          
  this%SinkFlows            => flowModel%SinkFlows            
  this%StorageFlows         => flowModel%StorageFlows         
  this%SubFaceFlowsComputed => flowModel%SubFaceFlowsComputed 
  this%BoundaryFlows        => flowModel%BoundaryFlows        
  this%SubFaceFlows         => flowModel%SubFaceFlows         
  
  ! Allocate buffers for reading array and list data
  this%ListItemBuffer => flowModel%ListItemBuffer
  this%ArrayBufferDbl => flowModel%ArrayBufferDbl
  this%ArrayBufferInt => flowModel%ArrayBufferInt
  ! END FLOWMODEL
...
!  
!  if(allocated(this%flowModel%SinkFlows)) then
!      cellCount = this%Grid%CellCount
!      do n = 1, cellCount
!          this%IBoundTS(n) = this%IBound(n)
!          this%Heads(n) = 0.0d0
...
!---------------------------------------------------------------------------------------------------------------
  ...
  this%TrackCell%SteadyState = this%flowModel%SteadyState
  ...
  do while(continueLoop)
      ...
      if(traceModeOn) then
         call WriteTraceData(traceModeUnit, this%TrackCell,                     &
           this%TrackCellResult, this%flowModel%GetCurrentStressPeriod(),       &
           this%flowModel%GetCurrentTimeStep())
      end if
      !! Write trace mode data if the trace mode is on for this particle
      !if(traceModeOn) then
